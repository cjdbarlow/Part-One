...
The time constant is the **time taken for a process to complete if it continued at its initial rate of change**. Time constants are similar to the half-life, but are **better suited when modeling** change in **exponential processes**^[Time constants are discussed in more detail under [respiratory time constants](time-constants.md).].
...
* $Cl = k.V_D$^[Since $k$ and $V_D$ are constants, clearance is also a constant.] in ml.min^-1^, or: 
...
* **First-order Kinetics**^[First-order kinetics: <img src="resources/first-order-kinetics.svg">]\
...
* **Zero-order kinetics**^[Zero-order kinetics: <img src="resources/zero-order-kinetics.svg">]\
